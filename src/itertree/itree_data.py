...
class iTData(OrderedDict):
    ...
    def data(self):
        '''
        delivers the super dict of this class (do not use for manipulations directly!
        :return: dict
        '''
        return super(iTData, self)
    ...
    def pop(self, key=__NOKEY__,value_only=True):
        ...
        :param key: key where the item should be poped out
        :param value_only: True - only value will be deleted model will be kept in iTreeData
                           False - whole model will be poped out
        :return:
        '''
        if value_only:
            item=super(iTData, self).__getitem__(key)
