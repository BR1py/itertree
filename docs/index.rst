...
Every node in the itertree (iTree object) contains two parts of stored information. First the related sub-structure (iTree-children) and second the related data of any type can be stored in the internal data structure of the object. 
...
* a iTree can contain linked items linking to other internal tree parts or to an external itertree file
...
The package is a pure python package and does not have any dependencies. But we have two recommandations which give the package additional performance:
...
The itertrees are build by adding iTree-objects to a iTree-parent-object. This means we do not have an external tree generator.
...
To add subitems we can address the child item also by index and then add the sub-item.
...
The iterators and find functions of itertree can use filter objects placed item_filters parameter to search for specific properties. 
...
At least the itertree can be stored and reconstructed from a file. We can also link an item to a specific item in a file or create internal links.
...
* The StopIteration exception must be handled in case of empty iterators. To make the handling a bit easier iTree delivers in most cases an empty list if we have no match. But in some cases (e.g. filter operations) the user will get an iterator even when the iterator is empty. In helpers the user can find an iterator empty check function (is_iterator_empty(iterator)).

* The user must also consider that an iterator can be consumed only one time. To reuse an interator multiple times you may have a look on itertools.tee().

To summarize this chapter: 

We decided to deliver only iterators (and not lists) to give the user the possibility to utilize the whole iterator power. If he really needs a list (in most cases for index access) he can convert to a list very easy and quick (via `list()` statement). But if we would directly deliver lists by default we would have a perfomance drop in all itertree filter functions which was not acceptable from our point of view.
